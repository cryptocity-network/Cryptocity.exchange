<template>
  <section class="relative h-[480px] md:h-[322px] rounded-3xl overflow-hidden bg-contain bg-right bg-crypto-map mb-10">
    <!-- Gradient Overlay -->
    <div class="absolute inset-0 bg-gradient-to-b md:bg-gradient-to-r from-[#1F2348] via-[#1F2348]/100 to-transparent">
    </div>

    <!-- Content -->
    <div class="relative z-10 h-full flex flex-col justify-center p-8 max-w-md">
      <!-- Map Icon -->
      <div class="md:pl-14">
        <div class="mb-6">
          <div class="w-12 h-12 flex items-center justify-center mx-auto md:mx-0">
            <img src="/icon-cryptomap.svg" alt="Download on the App Store" class="h-12" />
          </div>
        </div>

        <!-- Text Content -->
        <h2 class="text-4xl font-bold text-white mb-4 text-center md:text-left">
          Crypto Map
        </h2>
        <p class="text-gray-300 text-lg mb-8 text-center md:text-left">
          Find places to spend Crypto
        </p>

        <!-- Explore Button -->
        <button class="nq-pill-blue px-8 py-2 font-bold text-xl mx-auto md:mx-0">
          <span>Explore</span>
          <div class="i-lucide-arrow-right text-lg"></div>
        </button>
      </div>

    </div>
  </section>
</template>

<script setup lang="ts">
// Component logic can be added here if needed
</script>

<style>
.bg-crypto-map {
  background-image: url('/bg-cryptomap.svg');
  background-size: contain;
  background-position: right;
}

@media screen and (max-width: 768px) {
  .bg-crypto-map {

    background-size: cover;
    background-position: bottom;
  }

}
</style>